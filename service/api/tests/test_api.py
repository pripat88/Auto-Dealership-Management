...
class Tests(TransactionTestCase):
    ...
    def test_appointment_list(self):
        ...
        self.assertEqual(
            response.status_code, 200, msg="Did not get a 200 OK for list appointment."
        )
        self.assertTrue(
            "appointments" in data, msg="Did not give response with appointment field."
        )
        self.assertEqual(
            len(data["appointments"]),
            1,
            msg="Did not return correct number of appointment.",
        )
    ...
    def test_appointment_create(self):
        ...
        self.assertTrue(
            response.status_code == 404 or response.status_code == 400,
            msg="Did not get a 400 appointments create with an unknown tech.",
        )
