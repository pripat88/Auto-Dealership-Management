   ...
   const handleComplete = async (event) => {
        ...
        if (appointmentResponse.ok){
            ...
            setAppointments(service => {
                return appointments.filter(appointment => appointment.id !== event.id)
            })
    ...
    async function deleteAppointment(appointments) {
        ...
        if (response.ok) {
            setAppointments(appointment => {
                return appointment.filter(appointment => appointment.id !== appointments.id)
            })
                ...
                <tbody>
                    ...
                    return (
                        <tr key={appointment.id} value={appointment.id}>
                            ...
                            <td>{ new Date (appointment.time).toLocaleTimeString([], { hour: "2-digit"}) }</td>
                            <td>{ appointment.technician.name}</td>
                            <td>{ appointment.reason }</td>
                            <td>
                                <button className="btn btn-danger" onClick={() => deleteAppointment(appointment)}>Cancel</button>
                                <button className="btn btn-success"onClick={() => handleComplete(appointment)} >Finished</button>
                            </td>
                        </tr>
                    );
                })}
